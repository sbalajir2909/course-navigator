...
async def on_startup() -> None:
    ...
    print(f"  LLM order:     Ollama local → Ollama remote → Cloudflare → OpenAI")
    print(f"  Ollama local:  {ollama_local}  (teach=llama3.1:8b, validate=qwen2.5:72b, course=llama3.1:8b)")
    print(f"  Ollama remote: {ollama_remote or '(not set — run scripts/gautschi_tunnel.sh)'}")
    print("=" * 60 + "\n")
