"""
api/routes/auth.py
Student authentication: register, login, get current student.
Uses simple password hashing (bcrypt) — no JWT needed for hackathon, use session token in localStorage.
"""
from __future__ import annotations
import hashlib, os, uuid, secrets
...
async def register(body: RegisterRequest):
    ...
    existing = await supabase_query("students", params={"email": f"eq.{body.email}", "select": "id,email,name"})
    if existing:
