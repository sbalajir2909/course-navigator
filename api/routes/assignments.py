...
async def _auto_grade(submission_text: str, rubric: list[dict]) -> dict:
    """Auto-grade a submission against each rubric criterion using Cloudflare AI."""
...
async def generate_assignments(course_id: str):
    ...
    module_mastery: dict[str, list] = {}
    for s in sessions:
        mid = s["module_id"]
        module_mastery.setdefault(mid, []).append(s.get("mastery_score", 0))
    ...
    student_masteries: dict[str, list] = {}
    for s in sessions:
        student_masteries.setdefault(s["student_id"], []).append(s.get("mastery_score", 0))
    ...
    if session_ids:
        ...
        for a in attempts:
            pain = (a.get("validator_scores") or {}).get("pain_point", "")
            if pain:
                pain_counts[pain] = pain_counts.get(pain, 0) + 1
        top_pain_points = sorted(pain_counts, key=lambda p: pain_counts[p], reverse=True)[:5]
